class GstCreditNoteRegisterController < ApplicationController
	 def index
    ...
    @ledgers = Ledger.by_voucher("GstCreditNote").get_record(@start_date, @end_date, @branch_id, @financial_year, @current_user, @account)
    prawnto :filename => "gst_credit_note_register.pdf"
  end

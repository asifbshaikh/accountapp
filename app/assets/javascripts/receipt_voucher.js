$(document).ready(function(){
  ...
  var advanceReceiptVoucherTable = $('#acvance_receipt_vouchers').dataTable({
    ...
    iDisplayLength:50,
      ...
      sAjaxSource: $('#acvance_receipt_vouchers').data('source')
   }).columnFilter({aoColumns:[
       { sSelector: ".recvoucherno" },
       { sSelector: ".recDate", type:"date-range"  },
       { sSelector: ".recacc", type:"select" },
       { sSelector: ".recproject", type: "select" },
       { sSelector: ".recamount", type: "number-range" }
       ]}
     );

   advanceReceiptVoucherTable.fnFilterOnReturn();

   var receiptvoucherTable = $('#receipt_vouchers').dataTable({
      sDom: "<'row'<'col-sm-8'f><'col-sm-4'>r>t<'row'<'col-sm-3'l><'col-sm-4'i><'col col-sm-5'p>>",
      sPaginationType: "full_numbers",
      iDisplayLength:50,
      bFilter: true,
      bProcessing: true,
      bServerSide: true,
      sAjaxSource: $('#receipt_vouchers').data('source')
   }).columnFilter({aoColumns:[
       { sSelector: ".recvoucherno" },
       { sSelector: ".recDate", type:"date-range"  },
       { sSelector: ".recacc", type:"select" },
   ...
   receiptvoucherTable.fnFilterOnReturn();
   ...
   if(accounts)  {
    $.each($.parseJSON(accounts), function(index, object){
      $(".recacc select").append("<option value="+object["id"]+">"+object["name"]+"</option>");
    })
   }
  var projects = $("td.recproject").attr('data-recproject')
   if(projects)  {
    $.each($.parseJSON(projects), function(index, object){
      $(".recproject select").append("<option value="+object["id"]+">"+object["name"]+"</option>");
    })
   }
  ...
  $("#receipt_vouchers_range_from_1").datepicker({format: 'dd-mm-yyyy'});
  $("#receipt_vouchers_range_to_1").datepicker({format: 'dd-mm-yyyy'});

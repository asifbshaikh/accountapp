class Withdrawal < ActiveRecord::Base
  ...
  def voucher_setting
    VoucherSetting.by_voucher_type(8, company_id).first
  end
  ...
  def validate_from_account_type
    if !(["BankAccount", "SecuredLoanAccount"]).include?(Account.find(from_account_id).accountable_type)
      errors.add(:from_account_id,"you entered is not a bank account, please select right account")
    end
  end
  def validate_to_account_type
     if !("CashAccount").include?(Account.find(to_account_id).accountable_type)
      errors.add(:to_account_id,"you entered is not a cash account, please select right account")
     end
  end
  ...
  class << self
    def new_record(company)
      withdrawal = Withdrawal.new
      withdrawal.company_id=company.id
      withdrawal.voucher_number = VoucherSetting.next_withdrawal_number(company)
      withdrawal
    end

    def create_record(params,company, user,fyr)
      withdrawal = Withdrawal.new(params[:withdrawal])
      withdrawal.from_account_id=nil if withdrawal.from_account.blank?
      withdrawal.to_account_id=nil if withdrawal.to_account.blank?
      withdrawal.company_id = company
      withdrawal.created_by = user.id
      withdrawal.branch_id = user.branch_id unless user.branch_id.blank?
      withdrawal.fin_year = fyr
      withdrawal
    end

    def update_record(params, company, user, fyr)
      withdrawal = Withdrawal.find(params[:id])
      withdrawal.assign_attributes(params[:withdrawal])
      withdrawal.from_account_id=nil if withdrawal.from_account.blank?
      withdrawal.to_account_id=nil if withdrawal.to_account.blank?
      withdrawal.branch_id = user.branch_id unless user.branch_id.blank?
      withdrawal.fin_year = fyr
...
#method for saving withdrawal with ledger
...
#method for updating withdrawal with ledger

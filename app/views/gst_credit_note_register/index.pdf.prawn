Prawn::Document.generate('gst_credit_note_register.pdf', :page => 'A4', :page_layout => :portrait)
	pdf.bounding_box([0,pdf.cursor], :width => pdf.bounds.width, :height =>80) do
		...
		pdf.text "<u>GST Credit Note Register</u>", :align => :center, :inline_format => true, :size => 12
	...
	for ledger in @ledgers
	  if !ledger.retrieve_corresponding_account.nil?
		data += [["#{(ledger.transaction_date.strftime("%d-%m-%Y") == prev_date)? "": ledger.transaction_date.strftime("%d-%m-%Y")}",
			"#{(ledger.debit > 0)? 'To':'By'} #{ledger.multiple_correlate_ledgers? ? "Multiple accounts" : Account.find(ledger.retrieve_corresponding_account).name}",
			"#{ledger.description}",
			"#{ledger.voucher_type}",

pdf = Prawn::Document.new(:page_layout => :portrait, :page => 'A4')
  ...
  pdf.text "<u>Journal Register</u>", :align => :center, :inline_format => true, :size => 12
     pdf.move_down 5
    pdf.text "#{(params[:account_id].blank?)? Account.find(@accounts.first.id).name  :
                   Account.find(params[:account_id]).name }", :align => :center, :inline_format => true, :size =>10
  ...
  data = [['Date','Voucher Number', 'Corresponding Account', 'P a r t i c u l a r s', {:content => 'Debit', :align => :right}, {:content => 'Credit', :align => :right}]]
  ...
  for ledger in @ledgers 
    if !ledger.retrieve_corresponding_account.nil?
      corresponding_account = Account.find(ledger.retrieve_corresponding_account)
      data +=[[{:content => "#{(ledger.transaction_date.strftime("%d-%m-%Y") == prev_date)? '': ledger.transaction_date.strftime("%d-%m-%Y")}"},
            ...
            {:content => "#{ledger.multiple_correlate_ledgers? ?  "Multiple accounts" : corresponding_account.name}"},

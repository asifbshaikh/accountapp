class GstrAdvanceReceiptPdf < Prawn::Document
  require 'open-uri'
  include ActionView::Helpers::NumberHelper
  include ApplicationHelper
  def initialize(gstr_advance_receipt, view_context, gstr_advance_receipt_line_items, tax_line_items, shipping_line_items)
    ...
    @company=@gstr_advance_receipt.company
    @view_context=view_context
   
    @created_by_user=User.find(@gstr_advance_receipt.created_by)
    @customer = @gstr_advance_receipt.from_account.customer.blank? ? @gstr_advance_receipt.from_account.vendor : @gstr_advance_receipt.from_account.customer
    @gstr_advance_receipt_line_items=gstr_advance_receipt_line_items
    @tax_line_items=tax_line_items
    @shipping_line_items =shipping_line_items
    @col_cnt=@view_context.gstr_advance_receipt_line_count-3
    @pos_arr=[]
    @header_pos=[]
    send "generate_gstr_advance_receipt"
  end



  def generate_gstr_advance_receipt
    ...
    bounding_box([0, (@header_pos.min.to_i-30)], :width=>bounds.width) do
      text "Advance Receipt", :align => :center, :inline_format => true, :size => 14
    ...
    bounding_box([2*box_width+20, y_offset], :width=>box_width) do
      table(gstr_advance_receipt_info, :width=>box_width, :row_colors=>["F2F7FF"], :cell_style=>{:align=>:left, :border_width=>0,:size=>8}) do
      end
      @pos_arr<<y
    end
    gstr_advance_receipt_rows
  ...
  def gstr_advance_receipt_info
    gstr_advance_receipt_info=[]
    gstr_advance_receipt_info<<[{:content=>"Advance Receipt #:", :font_style=>:bold, :align=>:left}, {:content=>"#{@gstr_advance_receipt.voucher_number}"}]
    gstr_advance_receipt_info<<[{:content =>"Advance Receipt date:", :font_style => :bold, :align => :left}, {:content =>"#{@gstr_advance_receipt.voucher_date.strftime("%d-%m-%Y")}"}]
    gstr_advance_receipt_info<<[{:content=>"Amount:", :font_style=>:bold, :align=>:left}, {:content =>"#{@gstr_advance_receipt.currency} #{format_amount(@gstr_advance_receipt.amount)}"}]
    #if !@custom_field.blank? && !@company.plan.free_plan?
      #if !@custom_field.custom_label1.blank? && !@gstr_advance_receipt.custom_field1.blank?
        #gstr_advance_receipt_info<<[{:content=>"#{@custom_field.custom_label1}", :font_style=>:bold, :align=>:left},{:content=>"#{@gstr_advance_receipt.custom_field1}"}]
      #end
      #if !@custom_field.custom_label2.blank? && !@gstr_advance_receipt.custom_field2.blank?
        #gstr_advance_receipt_info<<[{:content=>"#{@custom_field.custom_label2}", :font_style=>:bold, :align=>:left},{:content=>"#{@gstr_advance_receipt.custom_field2}"}]
      #end 
      #if !@custom_field.custom_label3.blank? && !@gstr_advance_receipt.custom_field3.blank?
        #gstr_advance_receipt_info<<[{:content=>"#{@custom_field.custom_label3}", :font_style=>:bold, :align=>:left},{:content=>"#{@gstr_advance_receipt.custom_field3}"}]
      #end 
    #end
    gstr_advance_receipt_info<<[{:content=>"Created by:", :font_style=>:bold, :align=>:left}, {:content=>"#{@gstr_advance_receipt.created_by_user}"}]
    gstr_advance_receipt_info
  end
  ...
  def bill_to
    ...
    billing_add<<[{:content=>"Bill To:", :font_style=>:bold, :align=>:left}]
      billing_add<<[{:content=>"#{@gstr_advance_receipt.from_account.name }", :font_style=>:bold, :align=>:left}]
  ...
  def ship_to
    ...
    shipping_add<<[{:content=>"#{@gstr_advance_receipt.from_account.name }", :font_style=>:bold, :align=>:left}]
    shipping_add<<[{:content=>"#{@customer.shipping_address.address_line1}", :align=>:left}]  unless @customer.billing_address.blank?
    shipping_add
  end

  def gstr_advance_receipt_items_header
    sub_data=[{:content=>"#{!@company.invoice_setting.item_label.blank? ? @company.invoice_setting.item_label : 'Item'}"},
      ...
      {:content=>"#{!@company.invoice_setting.amount_label.blank? ? @company.invoice_setting.amount_label : 'Amt'}(#{format_amount(@gstr_advance_receipt.currency)})", :align=>:right}]
  ...
  def product_rows
    ...
    @gstr_advance_receipt_line_items.each do |line_item|
      ...
      if @gstr_advance_receipt.get_discount>0
        sub_data.insert(4, {:content=>"#{number_with_precision line_item.discount_percentage, :precision=>2 }", :align=>:right})
      end
    ...
    sub_data=["",{:content => "Total Qty", :font_style => :bold, :align => :right},{:content => " #{ number_with_precision @gstr_advance_receipt.total_quantity, :precision=>2}", :font_style => :bold, :align => :right},"",""]
    unless @gstr_advance_receipt.get_discount>0
      sub_data.insert(4, "")
    end
  ...
  def tax_rows
    ...
    @tax_line_items.each do |line_item|
      sub_data=["", "", ""]
        ...
        sub_data.insert((@col_cnt+2), {:content => "#{number_with_precision @gstr_advance_receipt.group_tax_amt(line_item.account_id), :precision=>2}", :align => :right})
      line_items<<sub_data
    end
    line_items
  end
  ...
  def calculation_rows
    ...
    sub_data.insert((@col_cnt+1), "")
    sub_data.insert((@col_cnt+2), {:content => "#{number_with_precision @gstr_advance_receipt.sub_total, :precision=> 2}", :font_style => :bold, :align => :right})
    line_items<<sub_data
    ...
    if @gstr_advance_receipt.get_discount != 0
      ...
      sub_data.insert((@col_cnt+1), "")
      sub_data.insert((@col_cnt+2), {:content => "#{number_with_precision @gstr_advance_receipt.get_discount, :precision=>2}", :font_style => :bold, :align => :right})
      line_items<<sub_data
    end
    ...
    if  @gstr_advance_receipt.has_tax_lines?
      ...
      sub_data.insert((@col_cnt+1), "")
      sub_data.insert((@col_cnt+2), {:content => "#{ number_with_precision @gstr_advance_receipt.tax, :precision=>2}", :font_style => :bold, :align => :right})
      line_items<<sub_data
    end
    ...
    sub_data.insert((@col_cnt+1), "")
    sub_data.insert((@col_cnt+2), {:content => "#{number_with_precision @gstr_advance_receipt.amount, :precision=>2}", :font_style => :bold, :align => :right}) #BUG FIX for improper total in tax inclusive scenarios Author: Ashish Wadekar
    line_items<<sub_data
    line_items
  end

  def gstr_advance_receipt_rows
    line_items=Array.new
      line_items<<gstr_advance_receipt_items_header
      line_items=line_items.concat(product_rows)
   
    # @col_cnt= @gstr_advance_receipt.pdf_column_count
    unless @gstr_advance_receipt.tax==0
      line_items=line_items.concat(tax_rows)
    end
    line_items=line_items.concat(calculation_rows)
    
    self.y=@pos_arr.min.to_i-5
    n = @gstr_advance_receipt_line_items.count
    if  @gstr_advance_receipt.tax == 0 && @gstr_advance_receipt.get_discount==0
        column_widths = {2=>80, 3=>80, 4=>100 }
    elsif @gstr_advance_receipt.get_discount ==0
...
def gstr_advance_receipt_line_items
    lines = [["#{!@company.gstr_advance_receipt_setting.item.blank? ? @company.gstr_advance_receipt_setting.item : 'Item'}",
              "#{!@company.gstr_advance_receipt_setting.qty.blank? ? @company.gstr_advance_receipt_setting.qty : 'Qty'}",
              "#{!@company.gstr_advance_receipt_setting.rate.blank? ? @company.gstr_advance_receipt_setting.rate : 'Rate'}",
              "#{!@company.gstr_advance_receipt_setting.amount.blank? ? @company.gstradvance_receipt_setting.amount : 'Amount'}"]] +
    @gstr_advance_receipt_line_items.map do |line|
                [line.item_name, line.quantity, line.unit_rate, line.amount]
              end
              lines=lines +
                @tax_line_items.map do |line|
                ["", "", line.account.name.chomp("on sales"), @gstr_advance_receipt.group_tax_amt(line.account_id)]
              end
              lines=lines + [["", "", "Sub total", @gstr_advance_receipt.sub_total]]
              if @gstr_advance_receipt.discount != 0
                lines=lines + [["","","Discount",@gstr_advance_receipt.discount]]
              end
              if @gstr_advance_receipt.tax!= 0
                lines=lines + [["","","Tax",@gstr_advance_receipt.tax]]
              end
              lines= lines+[["","","Total",@gstr_advance_receipt.total_amount]]
  end

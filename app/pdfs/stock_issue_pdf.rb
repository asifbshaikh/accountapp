class StockIssuePdf < Prawn::Document
require 'open-uri'
include PdfBase
include ActionView::Helpers::NumberHelper
  
def initialize(view_context, stock_issue_voucher,company)
  super(PAGE_LAYOUT)
    @view_context=view_context
    @stock_issue_voucher=stock_issue_voucher
    @pos_arr=[]
    @header_pos=[]
    @company=company
    send "generate_stock_issue_voucher"
  end


def generate_stock_issue_voucher
    y_pos=cursor
    ...
    bounding_box([0, (@header_pos.min.to_i-30)], :width=>bounds.width) do
      text "<b>Stock Issue Voucher</b>", :align => :center, :inline_format => true, :size => 14
  ...
  bounding_box([0, (@pos_arr.min.to_i-50)], :width => (bounds.width)) do
    n =  @stock_issue_voucher.stock_issue_line_items.count
  ...
  def issue_details
    data=[[{:content =>"Voucher Number:", :font_style => :bold, :align => :left}, {:content =>"#{@stock_issue_voucher.voucher_number}"}]]
    data<<[{:content =>"Voucher Date :", :font_style => :bold, :align => :left}, {:content =>"#{ @stock_issue_voucher.voucher_date}"}]
    data<<[{:content =>"Godown:", :font_style => :bold, :align => :left}, {:content =>"#{ @stock_issue_voucher.stock_issued_warehouse}"}]
    data<<[{:content =>"Created On:", :font_style => :bold, :align => :left}, {:content =>"#{ @stock_issue_voucher.created_at.strftime("%d-%m-%Y")} by #{@stock_issue_voucher.created_by_user}"}]
